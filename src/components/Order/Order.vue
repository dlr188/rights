  ...
  <div>
    ...
    <el-dialog title="修改地址" :visible.sync="addressDialogVisible" width="50%" @close="addressDialogClosed">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="addressDialogcomfirm">确 定</el-button>
      </span>
    </el-dialog>
    <!-- 展示物流进度对话框 -->
...
export default {
  ...
  methods: {
    async getOrderList () {
      const { data: res } = await this.$http({url:'orders',params:this.queryInfo})
    ...
    // 修改地址 为提供接口
    addressDialogcomfirm(){
      this.addressDialogVisible = false
    },
    async showProgressDialog () {
      // 供测试的物流单号：1106975712662
      const { data: res } = await this.$http({url:'/kuaidi/1106975712662'})
      if (res.meta.status !== 200) {
